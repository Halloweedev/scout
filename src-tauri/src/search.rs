...
fn rebuild_index_sync(root: Option<String>) -> Result<IndexStatus, String> {
        ...
        .filter_entry(|entry| !skipped_directory(entry))
    {
        ...
        transaction
            .execute(
                "INSERT INTO entries (
                   path, name, parent, kind, extension, modified_ms, size, generation
    ...
    transaction
        .execute(
            "INSERT INTO meta(key, value) VALUES('last_indexed_ms', ?1)
             ON CONFLICT(key) DO UPDATE SET value = excluded.value",
...
fn score_candidate(candidate: Candidate, query: &str) -> Option<SearchResult> {
    ...
    if name_score <= 0.0 && path_score <= 0.0 {
        return None;
    }
    ...
    Some(SearchResult {
        ...
        score: name_score + path_score + directory_boost + usage_boost(candidate.opened_count, candidate.last_opened_ms),
    })
}
...
pub fn search_index(query: String, limit: Option<usize>) -> Result<Vec<SearchResult>, String> {
    ...
    let pattern = like_subsequence_pattern(&query);
    let mut statement = connection
        .prepare(
            "SELECT path, name, parent, kind, extension, opened_count, last_opened_ms
             FROM entries
        ...
        .map_err(|error| error.to_string())?;
    let rows = statement
        .query_map(params![pattern, SEARCH_CANDIDATE_LIMIT as i64], |row| {
            Ok(Candidate {
                path: row.get(0)?,
                name: row.get(1)?,
                parent: row.get(2)?,
